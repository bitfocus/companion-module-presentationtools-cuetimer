    ...
    if (self.config.host && self.config.port) {
      ...
      self.socket.on("data", (data) => {
        ...
        try {
          ...
          let tempFg = self.hexToRgb(
            jsonData.fg.substr(0, 1) + jsonData.fg.substr(3)
          );
          self.fgColor = combineRgb(tempFg.r, tempFg.g, tempFg.b);
          let tempBg = self.hexToRgb(
            jsonData.bg.substr(0, 1) + jsonData.bg.substr(3)
          );
    ...
    return [
      {
        ...
        label: "Information",
        value:
          "This will establish a TCP connection to interact with the CueTimer app",
      },
      {
        type: "textinput",
        id: "host",
    ...
    var feedbacks = {
      ...
      timerBackground: {
        ...
        callback: function (feedback) {
          if (feedback.options.Key in self.timers) {
            let tempBg = self.hexToRgb(
              self.timers[feedback.options.Key].bg.substr(1)
            );
